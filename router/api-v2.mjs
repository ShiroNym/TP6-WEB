...
import { getAllLinks, addLink, getLinkByShort, deleteAllLinks } from '../database/database.mjs';
    ...
    getAllLinks((err2, links) => {
      ...
      res.format({
        'application/json': () => {
          ...
          res.status(status).json({ short: link.short, url: link.url, shortUrl: `${req.protocol}://${req.get('host')}/api-v2/${link.short}`, duplicate: isDuplicate });
        },
        'text/html': () => {
...
router.get('/:short', (req, res) => {
  ...
  getLinkByShort(short, (err, link) => {
    ...
    res.format({
      'application/json': () => {
        res.json({ short: link.short, url: link.url, shortUrl: `${req.protocol}://${req.get('host')}/api-v2/${link.short}` });
      },
      'text/html': () => {
